...
public class WirelessPermissions {
	...
	private final String canTPToSign = "wirelessredstone.commands.tp";
	...
	public WirelessPermissions(final WirelessRedstone plugin) {
		...
		if (pm.getPlugin("Vault") != null
				&& WirelessRedstone.config.getVaultUsage()) {
			this.permissionsHandler = new Vault(plugin);
			permPlugin = "Vault";
			WirelessRedstone.getWRLogger()
					.info("Using Vault for permissions !");
			...
			WirelessRedstone.getWRLogger().info(
					"Using PermissionsEx for permissions !");
		} else if (pm.getPlugin("PermissionsBukkit") != null) {
			this.permissionsHandler = new SuperPerms(plugin);
			permPlugin = "PermissionsBukkit";
			WirelessRedstone.getWRLogger().info(
					"Using PermissionsBukkit for permissions !");
		} else if (pm.getPlugin("bPermissions") != null) {
			this.permissionsHandler = new SuperPerms(plugin);
			permPlugin = "bPermissions";
			WirelessRedstone.getWRLogger().info(
					"Using bPermissions for permissions !");
		} else if (pm.getPlugin("GroupManager") != null) {
			this.permissionsHandler = new SuperPerms(plugin);
			permPlugin = "GroupManager";
			WirelessRedstone.getWRLogger().info(
					"Using GroupManager for permissions !");
		} else {
			WirelessRedstone
					.getWRLogger()
					.info("None of the supported permissions plugins has been detected! Defaulting to OP/Config files!");
	...
	public boolean canTPToSign(final Player player) {
		return permissionsHandler.hasPermission(player, canTPToSign);
	}
}
